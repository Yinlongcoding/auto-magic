...
async function runSearchJob(job, request) {
  ...
  extraction.data.items.forEach((item) => delete item.cardIndex);
  await jumpToPageTop(resultTab.id);
  const detailResults = request.includeDetailFacts
    ? await captureAllRenderedDetails(job, extraction.data.items)
    : [];
...
function formatCompletionStep(data) {
  const count = Number(data?.count ?? data?.items?.length ?? 0);
  const details = Array.isArray(data?.detailResults) ? data.detailResults : [];
  if (!details.length) {
    return `采集完成：已获取 ${count} 条商品列表，未返回详情结果。`;
  }
  const success = details.filter((item) => item?.status === 'success').length;
  const partial = details.filter((item) => item?.status === 'partial').length;
  const failed = details.filter((item) => item?.status === 'failed').length;
  return `采集完成：列表 ${count} 条；详情已处理 ${details.length} 条（成功 ${success}、部分 ${partial}、失败 ${failed}）。`;
}
...
/* Dynamic links are resolved lazily by the detail queue; no list mutation,
   deduplication, or resolution report is produced. */
...
async function captureAllRenderedDetails(job, items) {
  const results = [];
